...
BOOL GetFileName(HWND hwnd, LPSTR pszFileName, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "C++ Files (*.cpp, *.cc)\0*.cpp\0C Files (*.c)\0*.c\0Header Files (*.h)\0*.h\0All Files (*.*)\0*.*\0\0";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	switch(Message) {
		case WM_CREATE: {
			/*static TCHAR A_1[] = TEXT("Button") ;
			static TCHAR A_2[] = TEXT("Complete") ;
			CreateWindow(A_1, A_2, WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
    					35, 80, 120, 60, hwnd, (HMENU)0, 0, NULL);*/
			...
			tbb[9].iBitmap = 0x0010;
			tbb[9].fsState = TBSTATE_ENABLED;
			tbb[9].fsStyle = TBSTYLE_BUTTON;
			tbb[9].idCommand = RN_CODE_TER ;
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				//case RN_CODE_COM:
				case HP_ABOUT:
					MessageBox(NULL, "This is an editor and It was made by Merron in 2021.\nWelcome to use!\nHave a good time!", "About", MB_OK | MB_ICONINFORMATION) ;
					break ;
				default: {
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				case IDM_SEARCH_FIND:
					SendMessage (g_hMDIClient, EM_GETSEL, 0, (LPARAM)&iOffset) ;
					hDlgModeless = PopFindFindDlg (g_hMDIClient) ;
					break ;
				case IDM_SEARCH_NEXT:
					SendMessage (g_hMDIClient, EM_GETSEL, 0, (LPARAM)&iOffset) ;
					
					if (PopFindValidFind())
						PopFindNextText (g_hMDIClient, &iOffset) ;
					else
						hDlgModeless = PopFindReplaceDlg (g_hMDIClient) ;
					break ;
